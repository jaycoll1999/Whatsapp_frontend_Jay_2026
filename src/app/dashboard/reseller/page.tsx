    ...
    useEffect(() => {
        const fetchAllData = async () => {
            try {
                ...
                // 1. Profile - Use analytics API for consistency with profile page
                const profileData = await resellerService.getProfile(token)
                
                // Use analytics API to get consistent data like profile page
                try {
                    ...
                    if (profileData.reseller_id) {
                        ...
                        // Use analytics data for consistency
                        const normalizedData = {
                            ...profileData,
                            wallet: {
                                total_credits: analyticsData.total_credits || profileData.wallet?.total_credits || 0,
                                used_credits: analyticsData.used_credits || 0,
                                available_credits: Math.max(0, (analyticsData.total_credits || 0) - (analyticsData.used_credits || 0))
                            }
                        };
                        
                        setData(normalizedData);
                    } else {
                        throw new Error('No reseller ID found');
                    }
                } catch (analyticsError) {
                    console.error('Failed to fetch analytics data, falling back to profile data:', analyticsError);
                    ...
                    const availableCredits = wallet.available_credits || wallet.credits_remaining || 0;
            ...
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-linear-to-br from-blue-600 to-blue-700 text-white p-6 rounded-3xl shadow-lg shadow-blue-600/20 relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            <p className="text-blue-100 font-medium mb-1 flex items-center gap-2">
                                <Wallet className="w-4 h-4" /> Total Credits Purchased
                            </p>
                            <h3 className="text-4xl font-bold tracking-tight">{Math.round(data?.wallet.total_credits ?? 0)}</h3>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 relative overflow-hidden group">
                        ...
                        <div>
                            <p className="text-gray-500 dark:text-gray-400 font-medium mb-1">Remaining to Distribute</p>
                            <h3 className="text-3xl font-bold text-gray-900 dark:text-white">{Math.round(data?.wallet.available_credits ?? 0)}</h3>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 relative overflow-hidden group">
                        ...
                        <div>
                            ...
                            <h3 className="text-3xl font-bold text-gray-900 dark:text-white">{Math.round(data?.wallet.used_credits ?? 0)}</h3>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="bg-gray-50 dark:bg-gray-900/50 p-6 rounded-2xl border border-gray-100 dark:border-gray-800 text-center">
                            <p className="text-gray-500 mb-2">Messages Sent (All Users)</p>
                            <h3 className="text-4xl font-bold text-gray-900 dark:text-white">
                                {(analytics as any)?.messages_sent ?? 0}
                            </h3>
                        </div>
                        <div className="bg-gray-50 dark:bg-gray-900/50 p-6 rounded-2xl border border-gray-100 dark:border-gray-800 text-center">
                            <p className="text-gray-500 mb-2">Credits Distributed</p>
                            <h3 className="text-4xl font-bold text-gray-900 dark:text-white">{Math.round(data?.wallet.used_credits ?? 0)}</h3>
                        </div>
                    </div>
                </div>
                                ...
                                topUsers.map(user => (
                                    <div key={user.busi_user_id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl">
                                        ...
                                        <div className="text-right">
                                            <p className="font-bold text-gray-900 dark:text-white">{Math.round(user.wallet.credits_used)}</p>
